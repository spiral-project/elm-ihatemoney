...
var author$project$Api$decodeProjectBill = A7(
	...
	A2(elm$json$Json$Decode$field, 'payer_id', elm$json$Json$Decode$int),
	A2(
		elm$json$Json$Decode$field,
		'owers',
		elm$json$Json$Decode$list(author$project$Api$decodeMember)));
var author$project$Types$BillsFetched = function (a) {
	return {$: 'BillsFetched', a: a};
};
var author$project$Api$fetchProjectBills = function (auth) {
	var projectID = function () {
		if (auth.$ === 'Basic') {
			var user = auth.a;
			return user;
	...
	return elm$http$Http$request(
		{
			...
			expect: A2(
				elm$http$Http$expectJson,
				author$project$Types$BillsFetched,
				elm$json$Json$Decode$list(author$project$Api$decodeProjectBill)),
			headers: _List_fromArray(
				[
					author$project$Api$headersForAuth(auth)
				]),
		...
		switch (msg.$) {
				...
				if (msg.a.$ === 'Ok') {
					...
					if (_n8.$ === 'Just') {
						...
						return _Utils_Tuple2(
							...
							author$project$Api$fetchProjectBills(model.auth));
					} else {
						return _Utils_Tuple2(
							_Utils_update(
								model,
			...
			case 'ProjectFetched':
				if (msg.a.$ === 'Ok') {
					...
					var err = msg.a.a;
					var _n16 = A2(elm$core$Debug$log, 'Error while loading the project', err);
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{auth: author$project$Types$Unauthenticated}),
			...
			case 'BillsFetched':
				if (msg.a.$ === 'Ok') {
					...
					var err = msg.a.a;
					var _n18 = A2(elm$core$Debug$log, 'Error while loading the project bills', err);
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{auth: author$project$Types$Unauthenticated}),
			...
			case 'BillUpdate':
				if (msg.b.$ === 'Ok') {
					...
					var bill = msg.a;
					var err = msg.b.a;
					var _n27 = A2(elm$core$Debug$log, 'Error while adding the bill', err);
					return _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
				}
			case 'BillDeleted':
				if (msg.b.$ === 'Ok') {
					var bill_id = msg.a;
					var _n28 = model.project;
					if (_n28.$ === 'Just') {
						var project = _n28.a;
...
var author$project$BillBoard$billInfoView = F3(
	function (t, members, bill) {
		var payerName = A2(
			elm$core$Maybe$withDefault,
			A4(author$project$Types$Member, 0, 'Unknown', 1, false),
			elm$core$List$head(
				A2(
					elm$core$List$filter,
					function (m) {
						return _Utils_eq(m.id, bill.payer);
					},
					members))).name;
		return A2(
			elm$html$Html$tr,
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							function () {
							var numberOfShares = elm$core$List$sum(
								A2(
									elm$core$List$map,
									function ($) {
										return $.weight;
									...
									bill.owers));
							var amountEach = A2(myrho$elm_round$Round$round, 2, bill.amount / numberOfShares);
...
var author$project$BillBoard$billBoardTable = F3(
	function (t, members, bills) {
		return A2(
			elm$html$Html$table,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					...
					A2(
						elm$core$List$map,
						A2(author$project$BillBoard$billInfoView, t, members),
						bills))
				]));
	});
var author$project$BillBoard$billBoardView = F3(
	function (t, members, bills) {
		return A2(
			elm$html$Html$div,
			_List_fromArray(
			...
			_List_fromArray(
				[
					...
					A3(author$project$BillBoard$billBoardTable, t, members, bills)
				]));
	});
...
var author$project$Locales$FR$getString = function (id) {
	switch (id.$) {
		...
		case 'ManageYourExpenses':
			return 'Gérez vos dépenses';
		case 'EasilyShared':
			return 'partagées, facilement';
		case 'TryDemo':
			return 'Essayez la démo';
		case 'SharingHouse':
			return 'Vous êtes en colocation ?';
		case 'GoingOnHoliday':
			return 'Partez en vacances avec des amis ?';
		case 'SimplySharingMoney':
			return 'Ça vous arrive de partager de l’argent avec d’autres ?';
		case 'WeCanHelp':
			return 'On peut vous aider !';
		case 'LogToExistingProject':
			return 'Se connecter à un projet existant…';
		case 'ProjectID':
		...
		case 'LogIn':
			return 'Se connecter';
		case 'CantRememberPassword':
			return 'Vous ne vous souvenez plus du code d’accès ?';
		case 'CreateNewProject':
			return '…ou créez en un nouveau';
		case 'ProjectName':
...
var author$project$SideBar$getMemberBalance = F2(
	function (member, bills) {
		var totalPaid = elm$core$List$sum(
			A2(
				elm$core$List$map,
				function ($) {
					return $.amount;
				...
				A2(
					elm$core$List$filter,
					function (bill) {
						return _Utils_eq(bill.payer, member.id);
					},
					bills)));
		var billsShares = elm$core$List$sum(
			A2(
				elm$core$List$map,
				function (bill) {
					return bill.amount / elm$core$List$sum(
						A2(
							elm$core$List$map,
							function ($) {
								return $.weight;
				...
				A2(
					...
					function (bill) {
						return A2(
							elm$core$List$any,
							function (ower) {
								return _Utils_eq(ower.id, member.id);
							},
							bill.owers);
					},
					bills)));
		var totalOwed = billsShares * member.weight;
